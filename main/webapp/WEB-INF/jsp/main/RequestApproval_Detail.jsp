	...
	<div class="sub_contents_wrap">	
		...
		<tbody style="text-align: center">
			<tr>
				...
				<td>${rqstUgcy.incdt_actvt_type_cd_nm }</td>
			</tr>
			<tr>
				<th>상세 내용</th>
				<td><span class="word_break">${rqstUgcy.ctlg_itm_nm }</span></td>
			<tr>
				<th>요청 사유</th>
				<td><c:out value="${rqstUgcy.rsn}"/></td>
			</tr> 	
			
			<tr>
				<th>검토 의견</th>
					<c:choose>
						<c:when test="${rqstUgcy.opn == null || rqstUgcy.opn == ''}">
							<c:choose>
								<c:when test="${fn:trim(rqstUgcy.state_cd) == 'D1'}">
									<td>검토 중</td>
								</c:when>
								<c:when test="${fn:trim(rqstUgcy.state_cd) == 'D2'}">
									<td>승인완료</td>
								</c:when>
								<c:otherwise>
									<td>반려</td>
